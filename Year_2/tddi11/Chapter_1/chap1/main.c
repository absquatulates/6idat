...
{	
	...
	while(true)
	{
		...
		temperatureMeasured_fractional &= 0x7; // Assignment
		// bits 13 to 17 are decimal part
		unsigned int temperatureMeasured_decimal = inputRegister >> 13 ; // Assignment
		temperatureMeasured_decimal &= 0x1F; // Assignment
		
		if (temperatureMeasured_decimal == 0x1F)
		{
			if(temperatureMeasured_fractional == 0x0)
				temperatureMeasured = -1;
			else
				temperatureMeasured = 31;
		}
		else
			temperatureMeasured = (float)temperatureMeasured_decimal + ((float)temperatureMeasured_fractional * 0.2); // Assignment
		...
		outputRegister |= (isHumidity2High_n<< 6);
		...
		outputRegister |= (isHumidity2Low_n << 7);
