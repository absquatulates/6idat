...
def ip():
    result = subprocess.run("cat /etc/network/interfaces | grep \"address 10.0.0.2/24\"", capture_output=True, shell=True, text=True)
    assert result.returncode == 0
...
def port22():
    result = subprocess.run("nc -zv server 22", shell=True, text=True)
    assert result.returncode == 0
    
def port80():
    result = subprocess.run("nc -zv server 80", shell=True, text=True)
