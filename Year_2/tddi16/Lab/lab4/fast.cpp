...
// Draw any lines that you find in 'points' using the function 'window->draw_line'.
    ...
    for (Point const& origo : points)
    {
        PolarSorter polarsorter{origo};
        sort(sorted.begin(), sorted.end(), polarsorter);
        for (auto it = sorted.begin(); it != next(sorted.end(), - 2); ++it)
            if (origo.sameSlope(*it, *next(it, 2), tolerance))
                window->draw_line({origo, *it, *next(it, 1)});
    }
